...
#define FONT_PATH       "/data/hisi_osd.ttf"
#define IntCalcAlign(n,align) ((n + align - 1) & (~(align - 1)))
#define OSD_BG_COROL          (0x0abc)
...
int string_to_bmp(char *pu8Str)
{
    ...
    font = TTF_OpenFont(FONT_PATH, 55); 
    if ( font == NULL ) 
    {  
    ...
    SDL_SaveBMP(temp, TIMER_BMP_PATH); 
...
int GetTimeString(char *string)
{
    ...
    printf("string: %s \n",s8Contenx);
    memcpy(string,s8Contenx,strlen(s8Contenx)+1);
