...
HI_S32 SAMPLE_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    ...
    printf("[%s][u32Width:%d][u32Height:%d][u32Square:%d]\n", __func__, u32Width, u32Height, u32Square);
    ...
    for (i=0; i<u32WndNum; i++)
    {
		...
		if(0==VoLayer)
		{
			switch(i)
			{
				...
				case 4:
				{
					return HI_SUCCESS;// just 0 1 2 3 forth channel
					break;
				}
...
HI_S32 SAMPLE_COMM_VO_BindVpss(VO_LAYER VoLayer,VO_CHN VoChn,VPSS_GRP VpssGrp,VPSS_CHN VpssChn)
{
	...
	printf("[%s:%d][VpssGrp(%d)VpssChn(%d)-->VoLayer(%d)VoChn(%d)\n", 
		__func__, __LINE__, VpssGrp, VpssChn, VoLayer, VoChn);
