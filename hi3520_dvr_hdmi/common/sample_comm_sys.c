...
{
    printf("[%s]设置Venc视频大小:enPicSize=0x%x,enNorm=0x%x\n",__func__, enPicSize,enNorm);
    ...
    switch (enPicSize)
    {
        ...
        case PIC_CIF:
            ...
            break;
        case PIC_HD1://ADD BY LIANZIHAO
            pstSize->u32Width = HD1_WIDTH;
            pstSize->u32Height = (VIDEO_ENCODING_MODE_PAL==enNorm)?HD1_HEIGHT:240;
            break;
        case PIC_D1:
...
HI_U32 SAMPLE_COMM_SYS_CalcPicVbBlkSize(VIDEO_NORM_E enNorm, PIC_SIZE_E enPicSize, PIXEL_FORMAT_E enPixFmt, HI_U32 u32AlignWidth,COMPRESS_MODE_E enCompFmt)
{
    ...
    {
        SAMPLE_PRT("PIXEL_FORMAT_YUV_SEMIPLANAR_422!\n", u32Width);
...
HI_S32 SAMPLE_COMM_SYS_Init(VB_CONF_S *pstVbConf)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_VB_SetConf failed!(%#x)\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_SYS_SetConf failed!(%#x)\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_SYS_Init failed!(%#x)\n", s32Ret);
        return HI_FAILURE;
    }
