...
#define G726_BPS G726_32K         /* MEDIA_G726_16K, MEDIA_G726_24K ... */

#define AUDIO_ADPCM_TYPE ADPCM_TYPE_DVI4/* ADPCM_TYPE_IMA, ADPCM_TYPE_DVI4*/

#define HI_ACODEC_TYPE_NVP6124	1
...
HI_S32 SAMPLE_NVP6134_CfgAudio(const AIO_ATTR_S *pstAioAttr,AcodecCgf_Type type)
{
	int fd;
	int tmp=0;
    nvp6134_audio_format format;
	nvp6134_aogain st_nvp6134_aogain;
	nvp6134_reg_wr st_nvp6134_reg_wr;
	if(NULL==pstAioAttr)
		goto SetOther;
    ...
    printf("[pstAioAttr->enWorkmode:%d]\n", pstAioAttr->enWorkmode);
    ...
    printf("[pstAioAttr->enSamplerate:%d]\n", pstAioAttr->enSamplerate);
...
SetOther:
	...
	if(AUDIO_PB_MUTE==type)
	{
		st_nvp6134_aogain.value=0;
		if (ioctl(fd, NVP6134_AOGAIN_SET, &st_nvp6134_aogain))
	    {
	        printf("func(%s) line(%d): ioctl NVP6134_AOGAIN_SET err!\n",
	                    __FUNCTION__, __LINE__);
	        close(fd);
	        return HI_FAILURE;
	    }
		st_nvp6134_reg_wr.reg=0x22;
		tmp=ioctl(fd, NVP6134_REG_READ, &st_nvp6134_reg_wr);
		printf("NVP6134_REG_READ:0x%x->value:0x%x\n",st_nvp6134_reg_wr.reg,tmp);
			
	}
	close(fd);
    printf("[%s:%d]SAMPLE_NVP6134_CfgAudio [type:%d] End!\n", __FUNCTION__, __LINE__,AUDIO_PB_MUTE);
    return HI_SUCCESS;
}
/************** Add By LianZiHao For NVP6134 2017-02-28 End **********************/

HI_S32 SAMPLE_NVP6124_CfgAudio(const AIO_ATTR_S *pstAioAttr,AcodecCgf_Type type)
{
	int fd;
    ...
    AcodecCgf_Type type, AD_TYPE_EN enCodecType)
{
	...
	if(AD_TYPE_NVP6134C == enCodecType)
    {
       /*** ACODEC_TYPE_NVP6134 ***/ 
       s32Ret = SAMPLE_NVP6134_CfgAudio(pstAioAttr,type);
       if (HI_SUCCESS != s32Ret)
       {
           printf("%s: SAMPLE_NVP6134_CfgAudio failed\n", __FUNCTION__);
           return s32Ret;
       }
       bCodecCfg = HI_TRUE;
    }

    if(AD_TYPE_NVP6124 == enCodecType)
    {
       /*** ACODEC_TYPE_NVP6124 ***/ 
       s32Ret = SAMPLE_NVP6124_CfgAudio(pstAioAttr,type);
       if (HI_SUCCESS != s32Ret)
       {
...
HI_S32 SAMPLE_COMM_AUDIO_AUDIO_Pb_Mute(AD_TYPE_EN enCodecType)
{
	...
	{
	   /*** ACODEC_TYPE_NVP6134 ***/ 
	   s32Ret = SAMPLE_NVP6134_CfgAudio(NULL,AUDIO_PB_MUTE);
	   if (HI_SUCCESS != s32Ret)
	   {
	       printf("%s: SAMPLE_NVP6134_CfgAudio failed\n", __FUNCTION__);
	       return s32Ret;
