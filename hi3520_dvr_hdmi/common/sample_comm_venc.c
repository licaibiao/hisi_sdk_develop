...
#if 1//add by lianzihao 2016-11-09====================================================>

static RATE_CTRL_T gs_stRCPara[32];
...
HI_S32 SAMPLE_COMM_VENC_SaveSnap(VENC_STREAM_S *pstStream)
{
	...
	//modify by lianzihao 2016-07-25 for test snap.
	sprintf(acFile, "/opt/usb_sd1_2/snap_%d.jpg", gs_s32SnapCnt);
...
#if 1//add by lianzihao 2017-11-28 ===================================================>

HI_S32 SAMPLE_COMM_VENC_Set_RCPara(VENC_CHN VencChn,
        RATE_CTRL_T *pstRCPara)
{
    if(NULL == pstRCPara)
    {   
        return -1;   
    ...
    memset(&gs_stRCPara[VencChn], 0, sizeof(RATE_CTRL_T));
    memcpy(&gs_stRCPara[VencChn], pstRCPara, sizeof(RATE_CTRL_T));

    return 0;
}

HI_S32 SAMPLE_COMM_VENC_Default_RCPara(VENC_CHN VencChn,
        RATE_CTRL_T *pstRCPara)
{
    if(NULL == pstRCPara)
    {   
    ...
    if((VencChn>0) && (VencChn<4))
    {
        pstRCPara->u32SetGop = 30;
        pstRCPara->u32SetFps = 15;
        pstRCPara->u32SetCBRBitRate = 1024*2;
        pstRCPara->u32SetVBRBitRate = 1024*2;
        pstRCPara->u32IQp = 10;
        pstRCPara->u32PQp = 40;
    }
    
    if((VencChn>4) && (VencChn<7))
    {
        pstRCPara->u32SetGop = 30;
        pstRCPara->u32SetFps = 12;
        pstRCPara->u32SetCBRBitRate = (1024+1024/2);
        pstRCPara->u32SetVBRBitRate = (1024+1024/2);
        pstRCPara->u32IQp = 15;
        pstRCPara->u32PQp = 50;
    }
...
VIDEO_NORM_E enNorm, PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode,HI_U32 u32Profile)
{
...
#if 1//add by lianzihao 2016-11-10 ===================================================>

    HI_U32 u32SetGop;
    HI_U32 u32SetFps;
    HI_U32 u32SetCBRBitRate;
    HI_U32 u32SetVBRBitRate;
    HI_U32 u32IQp;
    HI_U32 u32PQp;
    RATE_CTRL_T stDefaultRC;
	HI_U32 pu32MaxStrmCnt=48;

    SAMPLE_COMM_VENC_Default_RCPara(VencChn, &stDefaultRC);

    u32SetGop = gs_stRCPara[VencChn].u32SetGop ? gs_stRCPara[VencChn].u32SetGop : stDefaultRC.u32SetGop;
    u32SetFps = gs_stRCPara[VencChn].u32SetFps ? gs_stRCPara[VencChn].u32SetFps : stDefaultRC.u32SetFps;
    u32SetCBRBitRate = gs_stRCPara[VencChn].u32SetCBRBitRate ? gs_stRCPara[VencChn].u32SetCBRBitRate : stDefaultRC.u32SetCBRBitRate;
    u32SetVBRBitRate = gs_stRCPara[VencChn].u32SetVBRBitRate ? gs_stRCPara[VencChn].u32SetVBRBitRate : stDefaultRC.u32SetVBRBitRate;
    u32IQp = gs_stRCPara[VencChn].u32IQp ? gs_stRCPara[VencChn].u32IQp : stDefaultRC.u32IQp;
    u32PQp = gs_stRCPara[VencChn].u32PQp ? gs_stRCPara[VencChn].u32PQp : stDefaultRC.u32PQp;
    ...
    ******************************************/
    ...
    printf("[%s]设置Venc视频信息:enType=0x%x,enRcMode=0x%x\n",__func__, enType,enRcMode);
    switch(enType)
    {
        case PT_H264:
        {
            ...
            {
            	printf("[%s][VencChn:%d][u32SetGop:%d][u32SetFps:%d][u32SetCBRBitRate:%d][u32IQp:%d][u32PQp:%d]\n", 
            		__func__, VencChn, u32SetGop, u32SetFps, u32SetCBRBitRate, u32IQp, u32PQp);

                stVencChnAttr.stRcAttr.enRcMode = VENC_RC_MODE_H264CBR;
                stH264Cbr.u32Gop            = u32SetGop;
                stH264Cbr.u32SrcFrmRate      =25;
                stH264Cbr.fr32DstFrmRate = u32SetFps;
                stH264Cbr.u32BitRate = u32SetCBRBitRate;
                stH264Cbr.u32FluctuateLevel = 0;
                stH264Cbr.u32StatTime       = 1+(stH264Cbr.u32Gop/stH264Cbr.fr32DstFrmRate); 
                memcpy(&stVencChnAttr.stRcAttr.stAttrH264Cbr, &stH264Cbr, sizeof(VENC_ATTR_H264_CBR_S));
            }
            else if (SAMPLE_RC_FIXQP == enRcMode) 
            {
                stVencChnAttr.stRcAttr.enRcMode = VENC_RC_MODE_H264FIXQP;
                stH264FixQp.u32Gop =u32SetGop;
                stH264FixQp.u32SrcFrmRate =25;
                stH264FixQp.fr32DstFrmRate =u32SetFps;
            ...
            {
            	printf("[%s][VencChn:%d][u32SetGop:%d][u32SetFps:%d][u32SetVBRBitRate:%d][u32IQp:%d][u32PQp:%d]\n", 
            		__func__, VencChn, u32SetGop, u32SetFps, u32SetVBRBitRate, u32IQp, u32PQp);

                stVencChnAttr.stRcAttr.enRcMode = VENC_RC_MODE_H264VBR;
                stH264Vbr.u32Gop =u32SetGop;
                stH264Vbr.u32SrcFrmRate = 25;
                stH264Vbr.fr32DstFrmRate = u32SetFps;
                stH264Vbr.u32MinQp = u32IQp;
                stH264Vbr.u32MaxQp = u32PQp;
        	    stH264Vbr.u32MaxBitRate= u32SetVBRBitRate;
                stH264Vbr.u32StatTime       = 1+(stH264Vbr.u32Gop/stH264Vbr.fr32DstFrmRate); 
	...
	s32Ret = HI_MPI_VENC_SetMaxStreamCnt(VencChn,pu32MaxStrmCnt);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_VENC_SetMaxStreamCnt faild with%#x!\n", s32Ret);
        return HI_FAILURE;
    }
	SAMPLE_PRT("HI_MPI_VENC_SetMaxStreamCnt pu32MaxStrmCnt : %d !\n", pu32MaxStrmCnt);
	/******************************************
...
HI_S32 SAMPLE_COMM_VENC_Stop(VENC_CHN VencChn)
{
    ...
    ******************************************/
	...
	s32Ret =HI_MPI_VENC_ResetChn(VencChn);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_VENC_ResetChn faild with%#x!\n", s32Ret);
        return HI_FAILURE;
    }
