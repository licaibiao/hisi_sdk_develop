    ...
    PAYLOAD_TYPE_E enType, SIZE_S *pstSize, HI_S32 s32ChnNum)
{
    ...
    pstModVbConf->astCommPool[0].u32BlkSize = pstSize->u32Width * pstSize->u32Height * 2;
    pstModVbConf->astCommPool[0].u32BlkCnt  = 2*s32ChnNum;
...
HI_VOID	SAMPLE_WISDOM_VDEC_ChnAttr(VDEC_CHN_ATTR_S *pstVdecChnAttr,
    PAYLOAD_TYPE_E enType, SIZE_S *pstSize)
{
    if(NULL == pstVdecChnAttr)
        return;
    ...
    {
        pstVdecChnAttr->stVdecVideoAttr.enMode=VIDEO_MODE_STREAM;//VIDEO_MODE_FRAME;
        pstVdecChnAttr->stVdecVideoAttr.u32RefFrameNum = 2;// 1;
        pstVdecChnAttr->stVdecVideoAttr.bTemporalMvpEnable =0; //0;
    }
...
#if 0//add by lianzihao for test 2016-05-16
	printf("[%s:%d] [eCtrlSinal:%d] [s32StreamMode:%d] [enType:%d]\n ", 
		__func__, __LINE__, pstVdecThreadParam->eCtrlSinal,
		pstVdecThreadParam->s32StreamMode,
		pstVdecThreadParam->enType);
#endif			
...
#if 1//add by lianzihao for test 2016-05-16
	printf("[%s:%d]  [s32UsedBytes:%d]  [s32MinBufSize:%d] [s32ReadLen:%d]\n ", 
		__func__, __LINE__, s32UsedBytes, pstVdecThreadParam->s32MinBufSize, s32ReadLen);
#endif
...
HI_VOID SAMPLE_COMM_VDEC_StartSendStream(HI_S32 s32ChnNum, VdecThreadParam *pstVdecSend, pthread_t *pVdecThread)
{
    ...
    for(i=0; i<s32ChnNum; i++)
    {
        ...
        pthread_join(pVdecThread[i], NULL);
    }
}
...
HI_S32 SAMPLE_WISDOM_VDEC_Start(VDEC_CHN VdChn, VDEC_CHN_ATTR_S *pstAttr)
{
    HI_U32 u32BlkCnt = 10;
    ...
    if(1 == g_s32VBSource)
    {
        CHECK_CHN_RET(HI_MPI_VDEC_SetChnVBCnt(VdChn, u32BlkCnt), VdChn, "HI_MPI_VDEC_SetChnVBCnt");				
    }		

    CHECK_CHN_RET(HI_MPI_VDEC_CreateChn(VdChn, pstAttr), VdChn, "HI_MPI_VDEC_CreateChn");

    if (2 == g_s32VBSource)
    {
        stPool.hPicVbPool = g_ahVbPool[0];
        stPool.hPmvVbPool = -1;
        CHECK_CHN_RET(HI_MPI_VDEC_AttachVbPool(VdChn, &stPool), VdChn, "HI_MPI_VDEC_AttachVbPool");
    }

    CHECK_CHN_RET(HI_MPI_VDEC_StartRecvStream(VdChn), VdChn, "HI_MPI_VDEC_StartRecvStream");
    CHECK_CHN_RET(HI_MPI_VDEC_SetDisplayMode(VdChn, VIDEO_DISPLAY_MODE_PLAYBACK), VdChn, "HI_MPI_VDEC_SetDisplayMode");
...
HI_S32 SAMPLE_WISDOM_VDEC_BindVpss(VDEC_CHN VdChn, 
    VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
	...
	printf("[%s:%d][stSrcChn][s32DevId:%d][VdChn:%d]\n", 
		__func__, __LINE__, stSrcChn.s32DevId, VdChn);	
		
	printf("[%s:%d][stDestChn][VpssGrp:%d][VpssChn:%d]\n", 
		__func__, __LINE__, VpssGrp, VpssChn);

	iRet = HI_MPI_SYS_UnBind(&stSrcChn, &stDestChn);
	if(iRet != 0)
	{
        SAMPLE_PRT("HI_MPI_SYS_UnBind fail for %#x![srcChn:%d]\n", iRet, stSrcChn.s32ChnId);
	}
...
#if 1//add by lianzihao 20160514
	int iRet = 0;

	iRet = HI_MPI_SYS_UnBind(&stSrcChn, &stDestChn);
	if(iRet != 0)
	{
            printf("HI_MPI_SYS_UnBind fail for %#x![srcChn:%d]\n", iRet, stSrcChn.s32ChnId);
	}
#endif
		
	printf("[%s:%d][ stSrcChn.s32DevId:%d][stSrcChn.s32ChnId:%d]\n", 
		__func__, __LINE__,  stSrcChn.s32DevId, stSrcChn.s32ChnId);

	printf("[%s:%d][stDestChn.s32DevId:%d][stDestChn.s32ChnId:%d]\n", 
		__func__, __LINE__, stDestChn.s32DevId, stDestChn.s32ChnId);
...
HI_S32 SAMPLE_WISDOM_VDEC_UnBindVpss(VDEC_CHN VdChn, VPSS_GRP VpssGrp, VPSS_CHN VpssChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
